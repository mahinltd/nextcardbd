...
// --- 2. Import ALL necessary email functions ---
import { 
  sendNewOrderToAdmin, 
  sendCodOrderConfirmationToCustomer 
...
const createOrder = asyncHandler(async (req, res) => {
  ...
  // --- 8. (NEW STEP) SEND EMAILS FOR COD ---
  if (newOrder.status === 'Processing') { // i.e., it's a COD order
    // Find the customer details
    const customer = await User.findById(userId);
    if (customer) {
      // Send email to admin
      sendNewOrderToAdmin(newOrder).catch(err => console.error("Admin COD Email Error:", err));
      // Send email to customer
      sendCodOrderConfirmationToCustomer(newOrder, customer).catch(err => console.error("Customer COD Email Error:", err));
    }
  }
  // --- END OF NEW STEP ---
